  ...
  try {
    const [teachersData, subjectsData, classesData, roomsData, teacherRoomsData] =
      await Promise.all([
        ...
        supabase.from("teacher_rooms").select("teacher_id, room:rooms(*)"),
      ]);
...
const handleUpdateTeacher = async () => {
  ...
  try {
    ...
    // Delete existing teacher_rooms entries
    ...
    // Add new teacher_rooms entries
    if (editingTeacher.rooms && editingTeacher.rooms.length > 0) {
      const teacherRoomsToInsert = editingTeacher.rooms.map(room => ({
